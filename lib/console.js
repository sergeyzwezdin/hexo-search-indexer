...
const generateSearchIndex = (hexo) =>
    function () {
        ...
        return hexo.call('generate', {}).then(function () {
            ...
            const titles = config.content
                ? hexo.locals.get('posts').data.reduce((result, { path, title }) => {
                      result[path] = title;
                      return result;
                  }, {})
                : {};
            ...
            fs.writeFileSync(searchIndexPath, JSON.stringify(config.content ? { words, titles, contents } : words, null, 2), 'utf-8');
